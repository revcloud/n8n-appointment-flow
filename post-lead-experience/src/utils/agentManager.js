// Agent data mapping with ZIP codes
const AGENT_DATA = {
  robertH: {
...
// No default agent - if ZIP code not found, return null

/**
 * Get agent data based on ZIP code
 * @param {string} zipCode - The ZIP code to look up
 * @returns {Object|null} Agent data object or null if no match found
 */
export const getAgentByZipCode = (zipCode) => {
  ...
  // Return null if no match found - no default agent
  return null;
};

/**
 * Get agent data from URL parameters or localStorage
 * @returns {Object|null} Agent data object or null if no match found
 */
export const getCurrentAgent = () => {
  const urlParams = new URLSearchParams(window.location.search);

  // Try to extract ZIP from quiz_address first
...
/**
 * Extract ZIP code from URL-encoded address string
 * @param {string} encodedAddress - URL-encoded address string
 * @returns {string|null} - Extracted 5-digit ZIP code or null if not found
 * 
 * @example
 * extractZipFromAddress("123+Aetna+Street%2C+Mingo+Junction%2C+Ohio+43938%2C+United+States")
 * // Returns: "43938"
 */
